...
-- Fix auto_delete_listing_by_reports function to use SECURITY DEFINER
...
BEGIN
  ...
  IF v_report_count >= 10 THEN
    ...
    DELETE FROM car_listings WHERE id = v_listing_id;
  END IF;
...
-- Fix process_listing_report function to use SECURITY DEFINER
...
BEGIN
  ...
  IF p_status = 'approved' THEN
    ...
    -- If 10 or more reports, delete the listing directly
    IF v_report_count >= 10 THEN
      DELETE FROM car_listings WHERE id = v_listing_id;
    END IF;
  END IF;
