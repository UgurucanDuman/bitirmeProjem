-- Fix for damage reports system - drop existing policies first to avoid conflicts

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Users can create damage reports" ON damage_reports;
DROP POLICY IF EXISTS "Users can read their own damage reports" ON damage_reports;
DROP POLICY IF EXISTS "Anyone can read approved damage reports" ON damage_reports;
DROP POLICY IF EXISTS "Admins can manage damage reports" ON damage_reports;
DROP POLICY IF EXISTS "Users can create damage images" ON damage_images;
DROP POLICY IF EXISTS "Users can read their own damage images" ON damage_images;
DROP POLICY IF EXISTS "Anyone can read approved damage images" ON damage_images;
DROP POLICY IF EXISTS "Admins can manage damage images" ON damage_images;
DROP POLICY IF EXISTS "Anyone can read damage images" ON storage.objects;
DROP POLICY IF EXISTS "Users can upload damage images" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own damage images" ON storage.objects;
...
-- Create damage_images table if it doesn't exist
